...
/* ── SIDEBAR BODY ── */
.sb-body{padding:14px 12px;display:flex;flex-direction:column;gap:12px;flex:1;overflow:hidden;}
...
.sb-seed-label{font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:var(--text3);padding:6px 4px 3px;display:flex;align-items:center;gap:6px;}
...
.topbar{
  ...
  padding:0 28px;height:var(--topbar-h);flex-shrink:0;
...
.breadcrumb{font-size:11px;color:var(--text3);display:flex;align-items:center;gap:5px;min-width:100px;}
...
.tab-badge{display:inline-flex;align-items:center;justify-content:center;background:var(--danger);color:#fff;font-size:8px;font-weight:700;min-width:14px;height:14px;border-radius:7px;padding:0 3px;margin-left:4px;vertical-align:middle;}
...
.status-pill{display:flex;align-items:center;gap:5px;padding:4px 9px;border-radius:20px;border:1px solid rgba(255,255,255,0.2);background:rgba(13,21,32,0.3);backdrop-filter:blur(8px);font-size:10px;color:var(--text2);}
...
.auth-btn{padding:5px 13px;border-radius:7px;font-size:12px;font-weight:600;cursor:pointer;transition:all .18s;border:none;}
...
.kpi-card{
  background:rgba(255,255,255,0.06);backdrop-filter:blur(14px);border:1.5px solid rgba(255,255,255,0.12);border-radius:12px;padding:20px 22px;
...
.sug-chip{
  ...
  border-radius:14px;padding:18px 16px;text-align:left;cursor:pointer;font-size:12px;color:var(--text2);
  transition:all .25s var(--ease-out);line-height:1.4;display:flex;flex-direction:column;gap:10px;border-left:4px solid rgba(255,255,255,.18);
}
...
.welcome{margin:auto;display:flex;flex-direction:column;align-items:center;gap:32px;text-align:center;max-width:800px;width:100%;animation:welcomeIn .8s var(--ease-out) both;}
...
.rp-title{font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--text2);}
.rp-live{display:flex;align-items:center;gap:4px;font-size:9.5px;color:var(--danger);}
...
.rp-section{font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:var(--text3);}

.mini-chart-card{background:rgba(17,28,46,0.4);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:16px;}
[data-theme="light"] .mini-chart-card{background:rgba(240,245,255,0.4);border-color:rgba(0,0,0,0.06);}
.mini-chart-label{font-size:9px;color:var(--text3);margin-bottom:9px;letter-spacing:.04em;}
...
.asp-count{font-size:17px;font-weight:800;line-height:1;}
.asp-count.danger{color:var(--danger);}.asp-count.amber{color:var(--amber);}.asp-count.ok{color:var(--accent);}
.asp-label{font-size:8px;color:var(--text3);margin-top:2px;text-align:center;letter-spacing:.04em;}
...
/* ── CHANGE 1: greeting fires immediately at 0s (was 2.5s) ── */
.intro-greeting{
  ...
  animation-delay: var(--greeting-delay, 0s);
  margin-bottom:8px;
}
        ...
        <div className="data-table-wrap">
          <table className="full-table">
            ...
            <tbody>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--text3)", padding: "28px", fontSize: "11px" }}>No transactions match your filter.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
            ...
            <tbody>
              {filtered.map(m => (
                <tr key={m.name}>
                  ...
                  <td style={{ color: m.growth.startsWith("+") ? "var(--accent)" : "var(--danger)", fontWeight: 700 }}>{m.growth}</td>
  ...
  <div className="tab-page">
    ...
    <div className="full-table-wrap">
      <div className="full-table-head">
        ...
        <span style={{ fontSize: 11, color: "var(--danger)" }}>● LIVE</span>
      </div>
      <div className="data-table-wrap">
        <table className="full-table">
...
// ── Chat Message ──
// ── CHANGE 2: bubble-meta (time + copy) moved to BOTTOM of bubble ──
...
// ── CHANGE 1: greeting fires at 0s; description words start after greeting (~1s) ──
const SplitTextIntro: FC<{ onDone: () => void }> = ({ onDone }) => {
  ...
  return (
    <div className={`intro-text-block${fadeOut ? " fade-out" : ""}`}>
      {/* Greeting fires FIRST at 0s */}
      <h1 className="intro-greeting" style={{ '--greeting-delay': '0s' } as any}>
        Hi, I'm NEXUS.
      </h1>

      {/* Description lines start AFTER greeting (~1s) */}
...
export default function NEXUS({ introReady }: { introReady?: boolean }) {
  ...
  const sendMessage = async (text: string): Promise<void> => {
    ...
    try {
      ...
      if (err instanceof Error && err.name === "AbortError") { console.log("Query generation stopped by user."); return; }
